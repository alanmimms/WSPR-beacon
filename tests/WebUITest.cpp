...
public:
          ...
          server()
    {
        ...
        mockTimer.setTimeAcceleration(10); // 10x speed for demo
    ...
    void start() {
        ...
        std::cout << "Time acceleration: " << mockTimer.getTimeAcceleration() << "x\n";
...
private:
    ...
    void onTransmissionStart() {
        BandConfig currentBand = getCurrentBand();
        std::cout << "[BEACON] Transmission START on " << currentBand.name 
    ...
    void onTransmissionEnd() {
        transmissionCount++;
        BandConfig currentBand = getCurrentBand();
        std::cout << "[BEACON] Transmission END on " << currentBand.name 
                  << " (" << std::fixed << std::setprecision(6) << currentBand.frequency 
    ...
    void onStateChanged(FSM::NetworkState, FSM::TransmissionState) {
        std::cout << "[FSM] State: " << fsm.getNetworkStateString() 
