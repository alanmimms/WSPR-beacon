...
char *Settings::toJsonString() const {
  // Merge defaults and user into a new cJSON object and print
  ...
  cJSON_Delete(merged);
  return json; // caller must free() the result
}
...
bool Settings::fromJsonString(const char *jsonString) {
  ...
  // We only allow updates to keys present in defaults or user
  cJSON *it = nullptr;
